...
# IMPORTANTE: IT ONLY ALLOWS CHANGE THE NUMBER OF PEOPLE ON THE SYSTEM, NOT THE MAP
# Name of the policies ready to run: 
POLICY_NAME_FOLDER = ["baseline_1000", "baseline_10000", "baseline_25000"]
PEOPLE_GLOBAL = [1000, 10000, 25000]
CSV_CLEAN_FOLDER = Path("Fase_0/clean_csvs")

for i in range(3):

    BASE = Path(__file__).resolve().parent
    policy_name = POLICY_NAME_FOLDER[i] # Nome que as pastas vão ter e nome onde estão as coisas da policy
    ...
    # --------------------- POLICIES
    logistic_policy = {"id": policy_name, "type": "logistic", "L": 0.9, "k": 0.3, "x0": 3}
    utility_policy = {"id": policy_name, "type": "utility","base": 0.30,  "learning_rate": 0.35,        # inércia (0..1)
    "weights": {"time": -0.002, "cost": -0.30, "pollution": -0.000001}, "car_cost_per_km": 0.20, "bus_fare": 1.50}
    discrete_policy = {"id": policy_name, "type": "None", "base": 0.3}
    ...
    # --------------------- RESULTS OUTPUTS
    RESULTS_DIR = OUT_DIR / policy_name
    ...
    if __name__ == "__main__":
        ...
        print("ok1")
        df_clean = csvCleaner(policy_folder=RAW_CSV, policy_name=policy_name, clean=CSV_CLEAN_FOLDER)
        print("ok2")
        summary = summarize_daily_kpis(df_clean, policy_id=policy_name)
