...
for i in range(len(MAP_TYPE)):
    for j in range(len(PEOPLE_GLOBAL)):
        for l in range(len(SPECIFIC_MAP)):
            for k in range(len(POLICY_TYPE)):
                ...
                POLICY_NAME_FOLDER = f"{MAP_TYPE[i]}_{SPECIFIC_MAP[l]}_{POLICY_TYPE[k]}_{PEOPLE_GLOBAL[j]}"
                # Name of the folder that has the maps to use (folder with net, routes, etc)
                SCENARIO_FOLDER = f"{MAP_TYPE[i]}_map_{SPECIFIC_MAP[l]}"
                ...
                if __name__ == "__main__":
                    ...
                    print(f"- Specific Map: {SPECIFIC_MAP[l]}")
